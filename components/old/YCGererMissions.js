...
//-----------------------------------------
import CreateMission from './YCCreateMission';
import ModifyMission from './YCModifyMission';
import Mission from './YCMission';
...
function GererMissions() {
  ...
  function handleDeleteMission(idMission) {
    ...
    if (deleteRequestedFor === idMission) {
      ...
      axios
        .delete(`http://localhost:3000/missions/${idMission}`)
        .then((res) => {
          // Log du retour
          ...
          if (res.data.result) {
            ...
            toastDeleted({
              title: 'Demande Suppression Mission',
              description:
                "Nous n'avons pas pu supprimer la mission.",
              status: 'error',
              duration: 2000,
            });
          }
  ...
  if (client === "") {
    ...
    missions = missionsData.map((data, i) => {
      return (
        <Mission
          key={i}
          {...data}
